...
output "UI_URL" {
 ...
 output "UI_applicationName" {
   value = "${aws_codedeploy_deployment_group.UI[count.index].app_name}"
 }

  ## GET UI DeploymentGroup Name ##
 output "UI_DeploymentGroupName" {
   value = "${aws_codedeploy_deployment_group.UI[count.index].deployment_group_name}"
 }
...
output "API_URL" {
 ...
 output "API_applicationName" {
   value = "${aws_codedeploy_deployment_group.API[count.index].app_name}"
 }

  ## GET API DeploymentGroup Name ##
 output "API_DeploymentGroupName" {
   value = "${aws_codedeploy_deployment_group.API[count.index].deployment_group_name}"
 }
# output "alb_dns" {
